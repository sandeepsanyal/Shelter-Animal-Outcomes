...
##############################################################################################
# read train_data
train = read.csv("D:/Study/Case Studies/kaggle.com/Shelter-Animal-Outcomes-by-kaggle.com/original_data/train.csv", stringsAsFactors=FALSE)
Dataset = c(rep("train", length(train[,1])))
train=data.frame(train,Dataset, stringsAsFactors = FALSE)
# read test_data
test = read.csv("D:/Study/Case Studies/kaggle.com/Shelter-Animal-Outcomes-by-kaggle.com/original_data/test.csv", stringsAsFactors=FALSE)
test$ID = as.character(test$ID)
Dataset = c(rep("test", length(test[,1])))
test=data.frame(test,Dataset, stringsAsFactors = FALSE)
...
age_data = cbind(data$AnimalID, age_data)
...
age_data = cbind(age_data, as.numeric(temp))
...
age_data=data.frame(age_data, agecategory=temp)
...
age_data=subset(age_data, select=-c(ageNA))
...
animal_type_data = subset(data, select=c(AnimalID, AnimalType))
temp=unique(animal_type_data$AnimalType)
for(i in temp){
  animal_type_data[i]=as.numeric(ifelse((animal_type_data$AnimalType==i),1,0))
}
animal_type_data=subset(animal_type_data, select = -c(Cat))
...
library(stringr)
color_data=as.data.frame(str_split(data$Color, "/"),stringsAsFactors=FALSE)
temp=matrix(c(0),nrow(color_data),ncol(color_data))
for(i in 1:ncol(color_data)){
  temp[,i]=as.matrix(sort(color_data[,i]))
}
color_data=data.frame(data$AnimalID, t(temp),stringsAsFactors=FALSE);
...
temp1=as.data.frame(str_split_fixed(color_data$color1, " ",2),stringsAsFactors=FALSE)
temp1=as.data.frame(temp1[,-2],stringsAsFactors=FALSE)
temp2=as.data.frame(str_split_fixed(color_data$color2, " ",2),stringsAsFactors=FALSE)
temp2=as.data.frame(temp2[,-2],stringsAsFactors=FALSE)
color_data=data.frame(color_data$AnimalID, temp1, temp2)
colnames(color_data) = c("AnimalID", "color1", "color2")
...
temp = data.frame(AnimalID=color_data$AnimalID, color_data[,4:length(color_data)])
temp[is.na(temp)]=0
temp1=temp[c(2:length(temp))]
temp1=subset(temp1, select = -c(White))
color_data=data.frame(AnimalID=temp$AnimalID, color1=color_data$color1, color2=color_data$color2, temp1)
...
breed_data=data.frame(data$AnimalID, data$AnimalType, data$Breed)
...
temp=as.data.frame(str_split_fixed(temp1$Breed, "/",3),stringsAsFactors=FALSE)
temp1=data.frame(temp1$AnimalID,temp1$AnimalType, temp)
...
temp=as.data.frame(str_split_fixed(temp2$Breed, "/",2),stringsAsFactors=FALSE)
temp2=data.frame(temp2$AnimalID,temp2$AnimalType, temp)
...
temp = data.frame(AnimalID=breed_data$AnimalID, breed_data[,6:length(breed_data)])
temp[is.na(temp)]=0
temp1=temp[c(2:length(temp))]
temp1=subset(temp1, select = -c(Unknown))
breed_data=data.frame(AnimalID=temp$AnimalID, AnimalType=breed_data$AnimalType, breed1=breed_data$Breed1, breed2=breed_data$Breed2, breed3=breed_data$Breed3, temp1)
breed_data$AnimalID = as.character(breed_data$AnimalID)


########################### preparing final datasets for modelling ############################

data=data.frame(subset(data, select = c(AnimalID, OutcomeType)),
                Dog = animal_type_data$Dog,
                subset(age_data, select = c(age225, age365, age730, age1825, age8030)),
                subset(sex_data, select = -c(AnimalID, sex_subtype, sex)),
                subset(color_data, select = -c(AnimalID, color1, color2)),
                subset(breed_data, select = -c(AnimalID, AnimalType, breed1, breed2,breed3)),
...
set.seed(1)
forest = randomForest(x = training_data[,c(3:90)], y = training_data[,2], data = training_data, ntree = 1000, importance = TRUE)
print(forest)

# predict using testing_data
prediction = predict(object=forest, newdata=testing_data[,c(2:89)], type = "vote")

# creating the solution dataframe
solution = data.frame(ID = testing_data$ID, prediction)

# exporting solution to solution.csv
write.csv(solution, "D:/Study/Case Studies/kaggle.com/Shelter-Animal-Outcomes-by-kaggle.com/solution.csv", row.names=FALSE)
